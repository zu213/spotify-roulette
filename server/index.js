...
function checkPlayers() {
  ...
  tables.forEach(table => {
    table.playerIds = table.playerIds.filter(
      player => now - player.lastActivityTime < playerInactivityTimeout
    )
  })
}
    ...
    try {
      ...
      switch (data.type) {
        ...
        case 'start_round':
          ...
          console.log(`Started round on table ${tableCode}`)
          ...
          for(let i = 0; i<tables[tableIndex].playerIds.length; i++) tables[tableIndex].playerIds[i].playingGame = true
          break
        ...
        case 'guess_made':
          ...
          if(answerPlayer.id == data['playerId']) tables[tableIndex].playerIds[playerIndex].score += Number(data['timeLeft'])

          const gameInPlay = tables[tableIndex].playerIds.some(p => p.playingGame)
